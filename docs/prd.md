...
5. Step 1: 테이블 선택 (Base + Join 테이블들)
   ↓
6. Step 2: JOIN 타입 선택 (INNER/LEFT/RIGHT/FULL/CROSS)
   ↓
...
### 3. 추가 구현 완료 기능 (✅ v1.3.0)

| ID | 기능명 | 설명 | 관련 페이지 |
|----|--------|------|------------|
| **F007** | 고급 Join (최대 5개 테이블, CROSS JOIN) | 최대 5개 테이블 Join 지원 (Base + 4 Join Tables), INNER/LEFT/RIGHT/FULL/CROSS JOIN 타입 지원 | Join Query Builder 페이지 |
| **F008** | 데이터 필터링 | 서버 사이드 필터링 지원 (=, !=, >, <, >=, <=, LIKE 연산자) | Dataset 상세 페이지 |
| **F009** | 데이터 정렬 | 서버 사이드 정렬 지원 (ASC/DESC) | Dataset 상세 페이지 |
...
> **구현 기능:** `F002`, `F003`, `F006`, `F008`, `F009`, `F012` | **인증:** 필수 (로그인)

| 항목 | 내용 |
|------|------|
| **역할** | Dataset 데이터 뷰어 - 업로드된 데이터의 실제 내용 확인, 필터링, 정렬 및 검증 |
| **진입 경로** | Dataset 목록에서 **상세보기** 버튼 클릭 |
| **사용자 행동** | Dataset의 메타데이터와 실제 데이터 확인, 필터링/정렬 적용, 필요시 삭제 |
| **주요 기능** | • Dataset 메타데이터 표시 (이름, 행 수, 컬럼 수, 생성일시)<br>• 컬럼 정보 테이블 (컬럼명, 타입)<br>• 데이터 테이블 (TanStack Table, 클라이언트 정렬 가능)<br>• **서버 사이드 필터링** (컬럼별 조건: =, !=, >, <, >=, <=, LIKE)<br>• **서버 사이드 정렬** (ASC/DESC)<br>• 페이지네이션 (20개씩)<br>• **목록으로** 버튼<br>• **삭제** 버튼 (확인 다이얼로그) |
...
> **구현 기능:** `F004`, `F005`, `F007`, `F012` | **인증:** 필수 (로그인)

| 항목 | 내용 |
|------|------|
| **역할** | 핵심 작업 수행 - 복잡한 SQL 없이 직관적인 UI로 최대 5개 테이블 Join 및 Export |
| **진입 경로** | 헤더의 **Join Query Builder** 메뉴 클릭 |
| **사용자 행동** | 5단계 마법사를 통해 Join 조건 설정 → 실행 → CSV Export |
| **주요 기능** | • **Step 1**: Base 테이블 선택 + Join 테이블 추가 (최대 4개, 총 5개 테이블)<br>• **Step 2**: 각 Join 테이블별 JOIN 타입 선택 (INNER/LEFT/RIGHT/FULL/CROSS)<br>• **Step 3**: JOIN 조건 설정 (컬럼 매칭, 다중 조건 지원, CROSS JOIN은 조건 불필요)<br>• **Step 4**: Export할 컬럼 선택 (체크박스)<br>• **Step 5**: 실행 버튼 - Join 결과 미리보기 (페이지네이션 50개씩)<br>• **Export CSV** 버튼 - join_query_results_{timestamp}.csv 다운로드<br>• **리셋** 버튼 - 모든 설정 초기화 |
...
- Join Query Builder (5단계 마법사, 최대 5개 테이블)
- 고급 Join 지원 (INNER/LEFT/RIGHT/FULL/CROSS JOIN)
- 데이터 필터링/정렬 (서버 사이드)
- Join 결과 CSV Export
- JWT 인증 (로그인/로그아웃)
